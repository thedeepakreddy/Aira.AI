...
export default function Login({onComplete,onBack,reduceMotion}:{onComplete:()=>void;onBack:()=>void;reduceMotion:boolean}){
 ...
 return <section className="login-page screen-content"><div className="login-story"><div className="login-orb">{reduceMotion?<img src="/assets/orb.jpg" alt="Glowing orange orb"/>:<video src="/assets/orb.mp4" poster="/assets/orb.jpg" autoPlay loop muted playsInline aria-hidden="true"/>}</div><span className="eyebrow">AIRA BY ASKDEEPAKAI</span><h1>Big ideas.<br/>A little spark.</h1><p>Your conversations, your tools.<br/>One place to bring it all together.</p></div>
