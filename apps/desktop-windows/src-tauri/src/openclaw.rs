...
//!     user's own openclaw config if they have one.
...
fn find_binary() -> Option<String> {
    // On Windows, use where.exe and common npm install paths.
    crate::runtime::which("openclaw")
        .or_else(|| {
            ...
            if let Ok(localappdata) = std::env::var("LOCALAPPDATA") {
                candidates.push(format!("{localappdata}\\Programs\\openclaw\\openclaw.exe"));
            }
            candidates
                .into_iter()
...
fn state_dir() -> Result<std::path::PathBuf, String> {
    let dir = crate::runtime::aira_dir()?.join("openclaw");
...
pub const DENIED_TOOLS: &[&str] = &[
    ...
    // Writes to the user's filesystem.
