...
fn binary() -> Option<String> {
    // On Windows, search common install locations for whisper-server.exe.
    // scoop is the most common package manager that ships whisper.cpp.
    if let Ok(userprofile) = std::env::var("USERPROFILE") {
        let scoop = format!("{userprofile}\\scoop\\shims\\whisper-server.exe");
        if std::path::Path::new(&scoop).exists() {
            return Some(scoop);
        }
    }
    if let Ok(localappdata) = std::env::var("LOCALAPPDATA") {
        let local = format!("{localappdata}\\Programs\\whisper-cpp\\whisper-server.exe");
...
fn model_dir() -> Result<std::path::PathBuf, String> {
    let dir = crate::runtime::aira_dir()?.join("whisper");
...
pub fn voice_start(state: State<'_, VoiceState>) -> Result<VoiceStatus, String> {
    ...
    let binary = binary().ok_or_else(|| {
        "whisper.cpp is not installed. Install it with scoop or download from GitHub.".to_string()
    })?;
    let model = model_path()
...
mod tests {
    ...
    fn the_model_lives_under_aira_not_the_repo() {
        let dir = model_dir().expect("data dir");
